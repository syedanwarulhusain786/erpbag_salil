...
def accept_sales_order(request, sale_number):
    # Get the sales order object
    sales_order = get_object_or_404(Sales, sale_number=sale_number)

    # Update the sales order status to 'Accepted'
    sales_order.status = 'Accepted'
    sales_order.save()

    # Redirect to the sales order detail page or another appropriate page
    return redirect('sales_order_requirements', sales_id=sale_number)
...
def create_sales(request):
    ...
    context = {'services':services,'sales_number': next_quotation_number,'products':products,'Ledgers':Ledgers,'materials':material}
...
def sales_list(request):
    ...
    item_rows_with_sale = ItemRow.objects.select_related('sale').filter(sale__isnull=False,sale__user=user)
    print(item_rows_with_sale)
...
def create_quotation(request):
    ...
    context = {'quotation_number': next_quotation_number,'products':products,'material':material}
...
def quoteOrderList(request):
    ...
    if user.is_authenticated:
        company = user.company
    # Retrieve ItemRow objects with related Sale foreign key
    item_rows_with_sale = ItemRow.objects.select_related('quotation').filter(quotation__isnull=False,quotation__user=user)
    # Create a dictionary to store sale details along with related item rows
    sales_with_items = {}
    for item_row in item_rows_with_sale:
...
def sales_order_requirements(request, sales_id):
    sales_orders= ItemRow.objects.select_related('sale').filter(sale__sale_number=sales_id)
    status=Sales.objects.get(sale_number=sales_id).status
    print()
    final=[]
    count=0
    for order in sales_orders:
        ...
        for mat in material:
            ...
            )['total_quantity'] or 0  # Default to 0 if the total_quantity is None
            needed.append({'material':mat.material.name,
        ...
        'needed' :needed}) 
        count=count+1 
    return render(request, 'requirements.html',{'final':final,'sales_id':sales_id,'status':status})