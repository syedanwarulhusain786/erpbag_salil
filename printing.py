...
bot = telebot.TeleBot('6500766883:AAHCUj5PtG7sWJ3RnkP3X3ysGRf_gi35QaI')
...
def question(message):
    ...
    if user_exists(user_id):
        ...
        all=AllocatedMaterialProductionFlow.objects.filter(printing=True,cutting_status='Completed',printing_status='Pending')
        no_ans=[]
        for i in all:
...
def save(message,pk,callback):
    if message is not None:
        ...
        if (resp.last_update_printing+int(message.text))>resp.Material.rowItem.quantity:
            bot.reply_to(message, f'You Cant Produce quntity Greater than Left {resp.Material.rowItem.quantity-resp.last_update_printing}')
            send_options(message,pk)
            return 
        resp.last_update_printing+=int(message.text)
        resp.save()
        ...
        ProducedRow(
            ...
            productionUnit='Printing',
            TotalprocessedNow= message.text,
            TotalProcessed=resp.last_update_printing,
            TotalQuantity=resp.Material.rowItem.quantity,
            TotalLeft=resp.Material.rowItem.quantity-resp.last_update_printing,
        ).save()
        send_options(message,pk)
        resp=AllocatedMaterialProductionFlow.objects.get(pk=pk)
        if resp.Material.rowItem.quantity-resp.last_update_printing==0:
            bot.reply_to(message, f'Printing for This Product Is Completed Please Change Status')
...
def change_status(callback,kp):
    ...
    alc.printing_status='Completed'
    alc.save()
    bot.reply_to(callback.message, 'Status Of Allocation {alc.Material}  is Changed to Completed')
