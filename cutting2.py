...
bot = telebot.TeleBot('6946247168:AAGbMI_ebl-1_tLGTKn7BFNH_4LhEUSJ_6E')
...
def question(message):
    ...
    if user_exists(user_id):
        ...
        all=AllocatedMaterialProductionFlow.objects.filter(cutting=True,cutting_unit='Unit 2',cutting_status='Pending')
        no_ans=[]
        for i in all:
...
def send_produced(callback,kp):
    # bot.send_message(callback.message.chat.id, 'What is lighter?')
    resp=AllocatedMaterialProductionFlow.objects.get(pk=kp)
    header=['sales_order','product','TotalQuantity','TotalLeft']
    details=[header]
    res=ProducedRow.objects.filter(alloca=resp,productionUnit='Cutting')
    for r in res:
        details.append([r.sales_order.sale_number,r.product.name,str(r.TotalQuantity),str(r.TotalLeft)])
