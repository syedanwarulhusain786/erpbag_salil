...
bot = telebot.TeleBot('6452088948:AAGdGaPNBwmxqkYZcuKWDNhSj1LSLPPQ9uQ')
...
def question(message):
    ...
    if user_exists(user_id):
        ...
        all=AllocatedMaterialProductionFlow.objects.filter(stitching=True,stitching_unit='Unit 2',stitching_status='Pending')
        no_ans=[]
        for i in all:
...
def save(message,pk,callback):
    if message is not None:
        all=AllocatedMaterialProductionFlow.objects.filter(stitching=True,stitching_unit='Unit 1',stitching_status='Pending')
        for i in all:
            pk=i.pk
            resp=AllocatedMaterialProductionFlow.objects.get(pk=pk)
            if (resp.last_update_stitching+int(message.text))>resp.Material.rowItem.quantity:
                bot.reply_to(message, f'You Cant Produce quntity Greater than Left {resp.Material.rowItem.quantity-resp.last_update_stitching}')
                send_options(message,pk)
                return 
            resp.last_update_stitching+=int(message.text)
            resp.save()
            ...
            ProducedRow(
                ...
                productionUnit='Stitching',
                TotalprocessedNow= message.text,
                TotalProcessed=resp.last_update_stitching,
                TotalQuantity=resp.Material.rowItem.quantity,
                TotalLeft=resp.Material.rowItem.quantity-resp.last_update_stitching,
            ).save()
            
            resp=AllocatedMaterialProductionFlow.objects.get(pk=pk)
            if resp.Material.rowItem.quantity-resp.last_update_stitching==0:
                bot.reply_to(message, f'Stitching for This Product Is Completed Please Change Status ')
                bot.reply_to(message, f'Product Send To Inventory')
                
                change_status(callback,pk)
            send_options(message,pk)
        send_to_inventory(callback,pk,message.text)    
            # return
    else:
        pass
...
def change_status(callback,kp):
    ...
    alc.stitching_status='Completed'
    alc.save()
    bot.reply_to(callback.message, f'Status Of Allocation {alc.Material}  is Changed to Completed')
def send_to_inventory(callback,kp,qty):
    alc=AllocatedMaterialProductionFlow.objects.get(id=kp)
    material_stock = ProductStock(
    type='purchase',
    product=Product.objects.get(name=alc.Material.rowItem.product_name),
    content_type=ContentType.objects.get_for_model(AllocateMaterial),
    object_id=alc.Material.id,
    quantity=Decimal(qty),
    cost_of_single=Decimal(alc.Material.rowItem.unit_price),
    )
    material_stock.save()
    ReadyToDispatch.objects.get_or_create(
    alloca = alc,
    product_stock=material_stock,
    sales_order = alc.sales_order,
    product =Product.objects.get(name=alc.Material.rowItem.product_name),
    quantity = Decimal(qty),
    ledt=0,
    disp_left=0,
