...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function addNewRow() {
        ...
        const newRow = table.insertRow(table.rows.length - 4); // Insert before the last row
        ...
        const rowIndex = table.rows.length - 5; // Index of the new row (0-based)
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    table.addEventListener('input', function (event) {
        ...
        if (target.tagName === 'INPUT' && target.id.startsWith('ref') || target.id.startsWith('qty')) {
            updateAmount(target);
        }
    });
    ...
    function updateAmount(input) {
        ...
        var qtyInput = row.querySelector('input[id^="ref"]');
        var untInput = row.querySelector('input[id^="qty"]');
        ...
        // var cgst=d_total*0.09
        // var sgst=d_total*0.09
        // console.log(d_total)
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function calculateAndLogTotals() {
        ...
        const d_tax2 = document.getElementById("d_tax2");
