...
def material_stock_list(request):
    ...
    return render(request, 'productandservices/sd/material_stock_list.html', {'material_stocks': material_stocks})

def product_stock_list(request):
    product_stocks = ProductStock.objects.all()
    return render(request, 'productandservices/sd/product_stock_list.html', {'product_stocks': product_stocks})
...
def stock_report(request):
    # # Calculate total quantity, total cost, and total value for materials
    # material_report = MaterialStock.objects.aggregate(
    #     total_quantity=Sum('quantity'),
    #     total_cost=Sum('cost_of_all'),
    #     total_value=Sum('cost_of_all')
    # )

    # # Calculate total quantity, total cost, and total value for products
    # product_report = ProductStock.objects.aggregate(
    #     total_quantity=Sum('quantity'),
    #     total_cost=Sum('cost_of_all'),
    #     total_value=Sum('cost_of_all')
    # )
    # Calculate total quantity, total cost, and total value for materials (purchase - return)
    material_report = (
        ...
        .aggregate(
            total_quantity=Sum('quantity_value'),
            total_cost=Sum('cost_value'),
            total_value=Sum(F('cost_value') , output_field=DecimalField())
        )
    )

    # Calculate total quantity, total cost, and total value for products (purchase - return)
    product_report = (
    ...
    # Get the count of all material and product entries
    material_count = MaterialStock.objects.count()
    product_count = ProductStock.objects.count()

    # Calculate average cost per unit for materials and products
    try:
        average_cost_per_unit_material = (
            material_report['total_cost'] / material_report['total_quantity']
        ) if material_report['total_quantity'] > 0 else 0
    except:
        average_cost_per_unit_material=0
    try:
        average_cost_per_unit_product = (
            product_report['total_cost'] / product_report['total_quantity']
        ) if product_report['total_quantity'] > 0 else 0
    except:
        average_cost_per_unit_product=0
    # Material list query
    # Material list query
    # Material list query
    # material_list = (
    ...
    #     .annotate(
    #         total_quantity=Sum(ExpressionWrapper(F('quantity'), output_field=PositiveIntegerField())),
    #         total_value=Sum(ExpressionWrapper(F('cost_of_all'), output_field=DecimalField()))
    #     )
    # )
    ...
    context = {
        ...
        'average_cost_per_unit_material': round(average_cost_per_unit_material,2),
        'average_cost_per_unit_product': round(average_cost_per_unit_product,2),
